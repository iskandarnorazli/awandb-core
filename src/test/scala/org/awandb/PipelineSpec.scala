...
class PipelineSpec extends AnyFlatSpec with Matchers {
  ...
  // [UPDATED] Use a prime number to test unaligned tail chunks
  val ROWS = 10_000_037 
  val BATCH_SIZE = 4096 
  ...
  def runBenchmark(rows: Int, warmup: Boolean): Unit = {
    ...
    // Use Try-Finally for global cleanup
    try {
      ...
      try {
        // 1. Preload ALL data to C++ heap (Simulating Memory-Mapped Block)
        masterDataPtr = NativeBridge.allocMainStore(rows)
        NativeBridge.loadData(masterDataPtr, data)
        
        // 2. Allocate Result Buffer (Reusable)
        ...
        while (pos < rows) {
          ...
          // We slide the window over the master buffer without copying data
          ...
          // Sum hits (Scalar Reduce on SIMD Result)
          var k = 0
          while (k < len) {
        ...
        if (!warmup) {
          ...
          // [CRITICAL ASSERTION]
          // If this fails, the speedup is meaningless.
