...
extern "C"
{
    ...
    JNIEXPORT void JNICALL Java_org_awandb_core_jni_NativeBridge_destroyBlockNative(JNIEnv *env, jobject obj, jlong ptr)
    {
        if (ptr != 0)
        {
            BlockHeader *block = (BlockHeader *)ptr;

            // [CRITICAL FIX] Remove from BufferPool before physical destruction!
            BufferPool::get_instance()->unregister_block(block);
    ...
    {
        if (srcPtr == 0 || dstArray == nullptr || len <= 0)
            return;
